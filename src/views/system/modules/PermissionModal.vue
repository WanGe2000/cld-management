  ...
  >
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
       ...
       <a-form-item label="所属服务"   :labelCol="labelCol" :wrapperCol="wrapperCol">
              <j-search-select-tag
                placeholder="请选择服务"
                v-decorator="['itemId', {rules: [{required: true, message: '请选择服务!'}]}]"
                :dict="dicItem"
              ></j-search-select-tag>
            </a-form-item>
        <a-form-item
...
<script>
  ...
  export default {
    ...
    components: {Icons,JSearchSelectTag},
    data () {
      return {
        dicItem:"sys_item,name,id,del_flag=0 AND item_state=0",
        drawerWidth:700,
        treeData:[],
        treeValue: '0-0-4',
        title:"操作",
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.loadTree();
        let fieldsVal = pick(this.model,'name','perms','permsType','itemId','component','url','sortNo','menuType','status');
        this.$nextTick(() => {
