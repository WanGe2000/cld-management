    ...
    <a-form>
      <a-form-item
        ...
        <a-tooltip placement="topLeft" >
          <template slot="title">
            <span>您隶属于多门店，请选择当前所在门店</span>
          </template>
          <a-avatar style="backgroundColor:#87d068" icon="gold" />
        </a-tooltip>
        <a-select v-model="departSelected" :class="{'valid-error':validate_status=='error'}" placeholder="请选择登录门店" style="margin-left:10px;width: 80%">
        ...
        return new Promise(resolve => {
          ...
          getAction(url).then(res=>{
            ...
            if(res.success){
              ...
              this.departSelected = id
              this.departList  = departs
              if(this.currDepartName){
                this.currTitle ="门店切换（当前门店 : "+this.currDepartName+"）"
              }
      ...
      departOk(){
        ...
        putAction("/sys/selectDepart",obj).then(res=>{
          if(res.success){
            ...
            //console.log("---切换组织门店---userInfo-------",store.getters.userInfo.orgCode);
            this.departClear()
          }
        })
      },
      show(){
        ...
        this.loadDepartList().then(()=>{
          ...
          if(!this.departList || this.departList.length<=0){
            this.$message.warning("您尚未设置门店信息!")
            this.departClear()
          }
        })
      },
